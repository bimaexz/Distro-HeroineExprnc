  ...
  $(document).ready(function() {
    ...
    if (productsTable.length != 0) {
        ...
        $('#exportButton').on('click', function() {
          ...
          var ws_data = [['ORDER ID', 'CUSTOMER NAME', 'DATE', 'PAYMENT METHOD', 'TOTAL', 'STATUS']]; // Header

          // Add the rows to the ws_data array excluding the 'Actions' column
          table.rows({ search: 'applied' }).every(function(rowIdx, tableLoop, rowLoop) {
              var data = this.data();
              // Extract text from the 'Status' column which contains HTML
              var statusText = $(data[5]).text();
              ws_data.push([data[0], data[1], data[2], data[3], data[4], statusText]);
          });
